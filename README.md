...
|--------|-----------------------|--------------------------:|
| [Fortinet](docs/Fortinet.md) | FortiOS / FortiProxy SSL‑VPN | **14** |
...
![Exploited edge CVEs by vendor — CISA KEV 2020–2026; all six cluster at 8–14](assets/edge-kev-chart.png)

**The most important takeaway is the spread, not the order.** Under a consistent scope, every major edge vendor sits between **8 and 14** exploited edge CVEs over six years. **No vendor is dramatically cleaner than the others.** Anyone selling you "vendor X is secure, vendor Y isn't" on raw counts is selling you noise. (See [Caveats](#caveats-sources--corrections): counts still partly reflect installed base, which we cannot normalize away.)
...
1. **No major edge vendor is meaningfully safer than the others.** Six vendors, one consistent scope, **8–14 exploited edge CVEs each** over six years. Choosing a firewall on its CVE reputation is choosing between roughly equivalent exposure.
